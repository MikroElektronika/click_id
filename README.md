...
If you use one of the mentioned images, it is enough to simply plug the click into the mikroBUS socket, and Linux will automatically connect your click with the appropriate driver and expose the interface for working with it.  
...
**NOTE:** _To be sure that your [Click Board™](https://www.mikroe.com/click-boards) is supported look for the “ID” logo on the back side of the Click board and check the currently [supported](https://github.com/MikroElektronika/click_id/blob/generate_csv/clicks_status.csv) [Click Board™](https://www.mikroe.com/click-boards)._

**NOTE:** _Even though the dynamic system allows for Plug-and-Play, Click boards are not hot-swappable! When switching between Click boards, be sure to remove power from the device first in order to prevent damage to the Click board or the platform they are connected to._
...
* After you have applied the mikroBUS™ kernel driver onto the embedded Linux platform, perform the following:
  ...
  * Verify the mikroBUS™ kernel driver (link do drivera) is enabled by utilizing the following command in the Terminal: `ls /sys/bus/mikrobus/devices/`.
...
Currently, database of the click boards is much larger than the number of drivers in Linux, so it is sometimes necessary to add support for a click in Linux.

Support for a click in Linux consists of several steps:
...
* Checking if the module on the click already has a driver in the Linux kernel. If this is not the case, it is necessary to [develop a Linux driver](https://www.kernel.org/doc/html/v4.13/driver-api/index.html) for the respective module.

* Create a [ClickID manifest](#clickid-manifest) for the click board neccessary for host can correctly configure the click for loading.  [ClickID manifest](#clickid-manifest) file contains the necessary information for a specific Linux driver for the Click Board™. For reference, you can analyze the example manifest for the [OLEDC Click](https://www.mikroe.com/oled-c-click#/263-clickid-yes).
...
This section will describe the Click's manifest file format required for the ClickID driver in the Linux kernel. Click manifest is derived from the [Greybus manifest](https://raw.githubusercontent.com/cfriedt/greybus-for-zephyr/master/doc/GreybusSpecification.pdf).
